...
- `deploy/compose/.env.example` now documents `HELMDECK_ELEVENLABS_API_KEY`, `HELMDECK_FAL_KEY`, and `HELMDECK_PEXELS_API_KEY`. These keys have first-class vault auto-hydration (or will once #230 lands for pexels) but were absent from the example file an operator copies on first install, so the only way to discover them was via a CHANGELOG entry or a pack's "key not found" error message. (#229)

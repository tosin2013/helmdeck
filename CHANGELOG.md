...
- Every sidecar Dockerfile now ends with a CLI-surface sentinel block that runs `<tool> --version` and `--help | grep -q -- '--<flag>'` for every flag the corresponding Go pack passes by name. T-3 of ADR 037. Catches the failure mode that motivated the ADR — an upstream flag rename or rename-squat fails `docker build`, not the first pack invocation in prod. Tracks: `marp --stdin/--allow-local-files/--theme-set/--images/--pdf/--pptx/--html` (sidecar); `hyperframes render --resolution/--fps/--quality/--output` (sidecar-hyperframes, the case that originally bit us); plus `--version` smoke for `mmdc`, `ffmpeg`, `@playwright/mcp`, `pnpm`, `yarn`, `tsc`, `ts-node`, `eslint`, `prettier`, `vitest`, `pytest`, `ruff`, `mypy`. Discovered along the way: `sidecar-entrypoint.sh:85` still invokes `@playwright/mcp@latest` via `npx`, bypassing the pinned global install; tracked as a follow-up. (#214)

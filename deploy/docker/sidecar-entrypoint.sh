...
start_playwright_mcp() {
    ...
    # Use the @playwright/mcp installed globally and pinned in the image
    # (sidecar.Dockerfile, ARG PLAYWRIGHT_MCP_VERSION) rather than
    # `npx --yes @playwright/mcp@latest`. The npx form re-resolves @latest
    # over the network on every container start, which (a) floats the
    # version away from the pinned, sentinel-checked copy baked into the
    # image — defeating ADR 037's reproducibility guarantee — and (b)
    # fails closed on an air-gapped or egress-restricted host. The global
    # install exposes the `playwright-mcp` bin on PATH.

...
**v0.21.x → v0.22.0** (current headline release — *agents that work on free models, with memory*): non-breaking. Closes ADRs 047-050. New built-in orchestration meta-packs — `helmdeck.route`, `helmdeck.plan`, `helmdeck.memory_store`, `helmdeck.memory_forget` — bringing the gateway catalog to **52 packs** (42 without a gateway). New always-listed MCP resources: `helmdeck://routing-guide`, `helmdeck://my-defaults`, `helmdeck://my-memory`, `helmdeck://context-budgets`, `helmdeck://my-plans`. New `/memory` Routing Memory UI panel. The LLM context manager (`internal/llmcontext`, ADR 050) compacts catalog-heavy prompts so orchestration packs work on small/free models; per-model budgets are configurable. The OpenClaw memory bridge (ADR 048) exposes stored facts to OpenClaw `memory_search` via an optional embedding sidecar + QMD corpus endpoint. SQLite audit migrations are additive (`pack_history` / `pipeline_history` / `plan_history` categories). No removed fields, no closed-set value changes. If you skipped releases, also read the v0.14→v0.21 note below.

**v0.14.x → v0.21.0** (rolled up): non-breaking across the run. This window added the marketplace beta (v0.13/v0.15), `email.send` (v0.16-ish), `slides.outline` + `blog.rewrite_for_audience` and the `*-rewrite-blog` pipeline family that replaced the old `*-ground-blog` pipelines, `hyperframes.compose`, `github.get_issue` / `github.create_pr`, and the `swe.solve` coding pipelines (ADR 046). See [RELEASES.md](../RELEASES.md) for the per-version detail. All additive; the only catalog *removal* is at the pipeline level (the four `*-ground-blog` built-ins were superseded by `*-rewrite-blog`, which produce original posts rather than annotated transcriptions).

	...
	return func(ctx context.Context, ec *packs.ExecutionContext) (json.RawMessage, error) {
		...
		for i, s := range slides {
			...
			if hasNarration && s.Notes != "" {
				...
				// #141 + #429: enforce per-segment floor at ENCODE time
				// via the ffmpeg apad filter, not at audio-file-prep
				// time via avenc.PadAudioToMin. The pre-encode pad
				// path produced duration-stretched AAC packets at slide
				// boundaries (#429) — apad-during-encode generates the
				// silence as PCM samples that the AAC encoder writes
				// as normal-density 1024-sample frames. durations[i]
				// now records the TARGET segment duration (max of TTS
				// duration and minTurnSec); the per-segment encode
				// honors that via ffmpegEncodeOpts.AudioPadDur.
		...
		for i := range slides {
			...
			// abundant RAM.
			//
			// AudioPadDur = durations[i] makes apad a no-op when the
			// TTS audio is already at or above the target (apad's
			// whole_dur only extends; it never trims), so this is
			// safe to set unconditionally — see ffmpegEncodeOpts doc
			// for the #429 rationale.
			primaryOpts := ffmpegEncodeOpts{
				Threads:     slidesNarrateFfmpegThreads(),
				AudioPadDur: durations[i],
			}
			...
			if res.ExitCode != 0 {
				if rc, ok := classifyShellExitCode(res.ExitCode); ok && rc == packs.CodeResourceExhausted {
					...
					retryOpts := ffmpegEncodeOpts{Threads: "1", Preset: "veryfast", AudioPadDur: durations[i]}
...
type ffmpegEncodeOpts struct {
	...
	// AudioPadDur (seconds) extends the audio stream to at least this
	// duration via the apad filter and pins the segment's total
	// duration via -t. When 0, no padding is applied and -shortest
	// terminates at the audio input's natural end (legacy behavior).
	//
	// Issue #429 fix: previously the slides.narrate handler called
	// avenc.PadAudioToMin to pre-pad the audio MP3 with silence
	// generated by anullsrc → libmp3lame, then handed the padded MP3
	// to the per-segment encoder. The AAC re-encode of the pre-padded
	// silence produced one duration-stretched packet per slide
	// boundary (1 AAC frame carrying metadata claiming ~2s of duration
	// vs the natural ~23ms-per-1024-sample frame), pushing the audio
	// stream's container-claimed duration ~26s past the actual content
	// on a typical 14-slide deck (13 inter-slide pads × ~2s each).
	//
	// The audio PLAYED correctly (the decoder emitted the right
	// samples) but the container metadata over-claimed, propagating
	// to the video container, the SRT alignment, and the engagement
	// chapter timestamps. apad with whole_dur generates the silence
	// inline as PCM samples during the encode pass — the AAC encoder
	// then emits normal-density 1024-sample frames, and the
	// container-claimed duration matches the actual content.
	AudioPadDur float64
}
...
func encodeSegment(ctx context.Context, ec *packs.ExecutionContext, slideFile, audioFile, segFile, vf string, opts ffmpegEncodeOpts) (session.ExecResult, error) {
	...
	// Audio padding + explicit duration when AudioPadDur is set.
	// Without padding we keep the legacy -shortest behavior (terminate
	// at the natural audio end). With padding the apad filter extends
	// the audio to AudioPadDur seconds; -t pins the total segment
	// duration to the same value so the video loop also terminates
	// deterministically. See struct doc + #429 for the rationale.
	audioFilter := ""
	durationFlag := "-shortest"
	if opts.AudioPadDur > 0 {
		audioFilter = fmt.Sprintf("-af 'apad=whole_dur=%.3f' ", opts.AudioPadDur)
		durationFlag = fmt.Sprintf("-t %.3f", opts.AudioPadDur)
	}
	cmd := fmt.Sprintf(
		"ffmpeg -y -loop 1 -i %s -i %s -c:v libx264 -threads %s %s-tune stillimage "+
			"-c:a aac -b:a 192k %s-vf '%s' -pix_fmt yuv420p %s %s",
		shellQuote(slideFile), shellQuote(audioFile), opts.Threads, presetFlag,
		audioFilter, vf, durationFlag, shellQuote(segFile),
	)

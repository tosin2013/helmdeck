...
2. **New `hyperframes.lint` pack** wraps `hyperframes lint --json` as a pre-render diagnostic. Soft-surface by default (findings are the output); pass `strict:true` to gate downstream packs on a clean lint result. Slots into `builtin.scaffolded-narrated-video` between `attach_audio` and `render` — catches render-killing issues before burning the render budget.
